...
const ReplyMilestones: React.FC<ReplyMilestonesProps> = ({
  user,
  showPercentage = false,
}) => {
  const replyCount = user.replyCount || 0;
  const totalMessages = user.totalMessagesReceived || 0;
  ...
  const replyPercentage =
    totalMessages > 0 ? Math.round((replyCount / totalMessages) * 100) : 0;

  if (starCount === 0 && !showPercentage) return null;

  return (
    <div className="inline-flex items-center" title={starTitle}>
      {starCount > 0 && !showPercentage &&
      ...
      {showPercentage && totalMessages > 0 && (
        <span className="ml-2 text-sm">
          Replied to {replyPercentage}% of questions
        </span>
      )}
    </div>
  );
