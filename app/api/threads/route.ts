...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (exists) {
      return Response.json(
        { success: false, message: "thread already exists" },
        { status: 409 }
      );
    }
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    const thread = await ThreadModel.findOne({ userId: user._id, slug }).lean();
    if (!thread) {
      return Response.json(
    ...
    await ThreadModel.deleteOne(
      { _id: thread._id, userId: user._id },
      { session: sessionDb }
    );
    await MessageModel.deleteMany(
      { threadId: thread._id, userId: user._id },
      { session: sessionDb }
    );
