...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const res = await ThreadModel.updateOne({ _id: id, userId: user._id }, { $set: { isReplied: false } });
  if (res.matchedCount === 0) return Response.json({ success: false, message: 'Thread not found' }, { status: 404 });
  return Response.json({ success: true, message: 'Thread unarchived' }, { status: 200 });
}